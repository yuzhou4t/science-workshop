...
- Macrodatas issue/article-section pages for protected Chinese journal catalog pages such as `管理世界` and `南开管理评论`. These pages expose titles, authors, abstracts, and review-cycle notes, but they are discovery sources only. `管理世界` uses the discovered year/issue and article order to construct CNKI CJFD paid detail entries such as `GLSJ202605008`; `南开管理评论` uses the discovered year/issue to try an NCPSD official detail resolver and stays `needs_official_pdf` until that resolver returns matched official article pages.
