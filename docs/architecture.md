...
The macOS LaunchAgent is generated by `scripts/launchd-plist.mjs` and installed by `scripts/install-daily-launchd.mjs`. The installed job runs `scripts/run-daily-publish.mjs`, which commits only generated daily data and pushes `origin/main` after a successful workflow.
