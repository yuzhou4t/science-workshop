...
class PaperReadingWorkflow:
    ...
    def _formula_prompt(self, markdown: str) -> str:
        return (
            ...
            "1. 公式原文或指标名称。\n"
            "2. 每个符号的含义、单位、取值范围或角色。\n"
            "3. 变量构造与数据来源：变量如何从原始数据转换而来。\n"
            "4. 计算步骤：按先后顺序讲清怎么从输入得到结果。\n"
            "5. 白话解释：不用术语堆砌，说明这个公式回答什么问题。\n"
            "6. 统计关系或模型含义：例如回归、PC1、最短路径、网络指标、情景模拟等如何支撑结论。\n"
            "7. 假设与局限：公式成立依赖什么条件，材料没有说明时写“材料中未明确说明”。\n"
